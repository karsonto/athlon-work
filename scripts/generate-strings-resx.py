...
ENTRIES: dict[str, tuple[str, str]] = {
    ...
    "Harness_Mode_Agent": ("Agent", "Agent"),
    "Harness_Mode_Agent_Tooltip": ("完整 Agent 能力（无长期记忆与任务规划）", "Full agent capabilities (no long-term memory or task planning)"),
    "Harness_Mode_Coding": ("Coding", "Coding"),
    "Harness_Mode_Coding_Tooltip": ("启用长期记忆与任务规划", "Long-term memory and task planning enabled"),
    "Harness_Mode_Ask": ("Ask", "Ask"),
    "Harness_Mode_Ask_Tooltip": ("只读问答：禁用写文件、执行命令与子代理", "Read-only Q&A: file writes, command execution, and sub-agents disabled"),
    "Harness_Picker_SelectMode": ("选择 Agent 模式", "Select agent mode"),
