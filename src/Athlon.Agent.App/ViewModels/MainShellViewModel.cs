...
public partial class MainShellViewModel : ObservableObject, IDisposable, ISessionHost, INavigationService
{
    ...
    private async Task DownloadWorkspaceTreeNodeAsync(WorkspaceTreeNodeViewModel? node)
    {
        ...
        try
        {
            if (node.IsDirectory)
            {
                ...
                var (downloaded, skipped) = await DownloadRemoteDirectoryRecursiveAsync(
                    node.FullPath,
                    localRoot).ConfigureAwait(true);
            ...
            else
            {
                ...
                await _sshClient.DownloadFileAsync(node.FullPath, saveDialog.FileName).ConfigureAwait(true);
        ...
        bool hitNodeIsDirectory)
    {
        ...
        try
        {
            foreach (var localPath in localPaths)
            {
                ...
                {
                    var result = await UploadLocalFileAsync(localPath, remoteDirectory).ConfigureAwait(true);
                    if (result) uploaded++; else skipped++;
                }
                else if (Directory.Exists(localPath))
                {
                    var (up, skip) = await UploadLocalDirectoryRecursiveAsync(localPath, remoteDirectory).ConfigureAwait(true);
                    uploaded += up;
                    skipped += skip;
                }
            }
    ...
    private async Task<(int Downloaded, int Skipped)> DownloadRemoteDirectoryRecursiveAsync(
        ...
        string localDirectory)
    {
        ...
        await foreach (var entry in _sshClient.ListAsync(remoteDirectory).ConfigureAwait(true))
        {
            ...
            {
                var (childDownloaded, childSkipped) = await DownloadRemoteDirectoryRecursiveAsync(
                    entry.FullPath,
                    localChild).ConfigureAwait(true);
            ...
            if (File.Exists(localChild)
                && !_notifier.ConfirmYesNo("Shell_OverwriteTitle", "Shell_OverwriteLocalMessage", entry.Name))
            {
                skipped++;
                continue;
            }

            await _sshClient.DownloadFileAsync(entry.FullPath, localChild).ConfigureAwait(true);
            downloaded++;
        }
    ...
    private async Task<bool> UploadLocalFileAsync(string localPath, string remoteDirectory)
    {
        var fileName = Path.GetFileName(localPath);
        var remotePath = RemotePathNormalizer.Combine(remoteDirectory, fileName);
        var existing = await _sshClient.TryGetFileInfoAsync(remotePath).ConfigureAwait(true);
        if (existing is not null
            && !_notifier.ConfirmYesNo("Shell_OverwriteTitle", "Shell_OverwriteRemoteMessage", fileName))
        {
            return false;
        }

        await _sshClient.UploadFileAsync(localPath, remotePath).ConfigureAwait(true);
        return true;
    }

    private async Task<(int Uploaded, int Skipped)> UploadLocalDirectoryRecursiveAsync(
        string localDirectory,
        string remoteParentDirectory)
    {
        ...
        await _sshClient.CreateDirectoryAsync(remoteDirectory).ConfigureAwait(true);

        var uploaded = 0;
        var skipped = 0;
        foreach (var file in Directory.EnumerateFiles(localDirectory))
        {
            if (await UploadLocalFileAsync(file, remoteDirectory).ConfigureAwait(true))
            {
                uploaded++;
            }
            else
        ...
        {
            var (up, skip) = await UploadLocalDirectoryRecursiveAsync(child, remoteDirectory).ConfigureAwait(true);
            uploaded += up;
            skipped += skip;
        }
