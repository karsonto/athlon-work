...
function applyChatI18n() {
  ...
  // Sentinel-based infinite scroll no longer shows a labeled button.
...
function isQuietTool(name) {
  ...
  // (QuestionBar / plan card respectively).
...
/**
 * Render one store item into a detached row element.
 * @param {import('./timeline-store.js').TimelineItem} item
 */
  ...
  try {
    if (item.type === 'TOOL') {
      ...
      ['TOOL_CALL_ARGS', 'TOOL_CALL_END', 'TOOL_CALL_OUTPUT', 'TOOL_CALL_RESULT',
        'TOOL_APPROVAL_REQUEST', 'TOOL_APPROVAL_RESOLVED'].forEach(function (type) {
